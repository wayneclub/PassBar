...
export default function BrowsePage() {
  ...
  useEffect(() => {
    if (user?.id) getBrowseProgressByUser(user.id).then(setBrowseProgress);
  }, [user?.id]);
  ...
  // Filter chapters by learned status
  const filteredSubjects = useMemo(() => {
    if (learnFilter === 'all') return subjects;
    return subjects.map((subject) => ({
      ...subject,
      chapters: subject.chapters.filter((ch) => {
        const hasProgress = progressByChapter.has(ch.id);
        return learnFilter === 'learned' ? hasProgress : !hasProgress;
      }),
    })).filter((s) => s.chapters.length > 0);
  }, [subjects, learnFilter, progressByChapter]);
