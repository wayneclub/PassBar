...
/** Requests notification permission and subscribes the current device to push, saving it for the signed-in user. */
export async function enablePushNotifications(_userId: string): Promise<{ ok: boolean; error?: string }> {
  ...
  try {
    ...
    console.warn('[PassBar] Failed to save push subscription:', err);
...
export async function disablePushNotifications(): Promise<void> {
  ...
  await api.delete('/push/subscriptions', { endpoint }).catch(() => undefined);
}
