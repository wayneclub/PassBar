...
export async function evaluateUserBadges(_userId: string, data: AchievementData) {
  try {
    const unlockedIds = await api.post<string[]>('/attempts/achievements/evaluate', data);
    const idSet = new Set(unlockedIds);
    return BADGES.filter((badge) => idSet.has(badge.id));
  } catch (err) {
...
export async function fetchUserBadges(_userId: string) {
  try {
    const rows = await api.get<Array<{ badgeKey: string; unlockedAt: string }>>('/attempts/achievements');
    return rows.map((r) => ({ badge_key: r.badgeKey, unlocked_at: r.unlockedAt }));
  } catch (err) {
