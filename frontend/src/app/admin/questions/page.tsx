...
import { getQuestionReports, resolveQuestionReport, type QuestionReport, type ReportCategory } from '@/lib/question-reports';
...
function useTabLoaders() {
  const loadQuestionFeedback = useCallback(async (filterResolved: 'unresolved' | 'resolved' | 'all') => {
    const [reports, contentFeedback] = await Promise.all([
      getQuestionReports({
    ...
    if (questionIds.length > 0) {
      ...
      for (const q of qData) {
        questionsMap[q.id] = { question_text: q.questionText, subject: q.subject, chapter_name: q.chapterName };
      }
    }
