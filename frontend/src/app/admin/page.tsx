    ...
    async function load() {
      ...
      const [summary, reportRows] = await Promise.all([
        api.get<{
          ...
          activeToday: number;
          sessions: Array<{ startedAt: string | null; questionCount: number | null; status: string | null; userAgent: string | null; completedAt: string | null }>;
          pendingUsers: Array<{ id: string; fullName: string | null; email: string | null; createdAt: string | null }>;
        }>(`/admin/dashboard-summary?sinceDays=${range}`),
      ...
      const sessionRows = summary.sessions
        ...
        .map((s) => ({
          ...
          question_count: s.questionCount ?? 0,
      ...
      setTrend(trendData);
      setPendingUsers(summary.pendingUsers.map((u) => ({
        id: u.id, full_name: u.fullName, email: u.email, created_at: u.createdAt ?? '',
      })));
